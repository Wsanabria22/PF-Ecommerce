...
import { getCategories, clearProductDetail } from '../../redux/actions/productsActions';
import { useDispatch, useSelector } from 'react-redux';
import Swal from "sweetalert2"
import { Button, Card, CardContent, Container, Grid, Typography , TextField , Input } from '@mui/material';
...
export default function ModifyForm({ id, productDetails }) {
  ...
  return (
    ...
    <Container className={styles.mainDiv}>
      ...
      <form className={styles.form}>
        ...
        <CardContent>
            ...
            <Grid container spacing={4} xs={12}>
              <Grid item xs={12}>
              <TextField className={styles.input} fullWidth margin="dense" label="Product title..." value={input.title} name="title" type="text" onChange={(e) => handleInputChange(e)} />
              {errors.title && <span className={styles.errorSpan}>*{errors.title}</span>}
              </Grid>
              <Grid item xs={12}>
              <TextField className={styles.inputArea} fullWidth margin="dense" label="Description" multiline minRows={3} placeholder="Description..." value={input.description} name="description" onChange={(e) => handleInputChange(e)} />
              {errors.description && <span className={styles.errorSpan}>*{errors.description}</span>}
              </Grid>
              <Grid item xs={12}>
              <TextField className={styles.input} fullWidth margin="dense" label="Price" placeholder="00" value={input.price} name="price" type="text" onChange={(e) => handleInputChange(e)} />
              {errors.price && <span className={styles.errorSpan}>*{errors.price}</span>}
              </Grid>
              <Grid item xs={12}>
              <TextField className={styles.input} fullWidth margin="dense" label="Rating" placeholder="0,00 to 5,00" value={input.rating} name="rating" type="number" min={0} max={5} step={0.01} onChange={(e) => handleInputChange(e)} />
              {errors.rating && <span className={styles.errorSpan}>*{errors.rating}</span>}
              </Grid>
              <Grid item xs={12}>
              <TextField className={styles.input} fullWidth margin="dense" label="Stock" placeholder="0" value={input.stock} name="stock" type="number" onChange={(e) => handleInputChange(e)} />
              {errors.stock && <span className={styles.errorSpan}>*{errors.stock}</span>}
              </Grid>
              <Grid item xs={12}>
              <TextField className={styles.input} fullWidth margin="dense" label="Brand" placeholder="Brand..." value={input.brand} name="brand" type="text" onChange={(e) => handleInputChange(e)} />
      ...
      <Card>
        <CardContent>
            <Typography variant="h5" align="left">Images Collection</Typography> 
            <hr />
            <Grid container spacing={4} xs={12}>
              <Grid item xs={12}>
                  <TextField className={styles.input} fullWidth margin="dense" label="Thumbnail image link" placeholder="https://..." value={input.thumbnail} name="thumbnail" type="text" onChange={(e) => handleInputChange(e)} />
                  {errors.thumbnail && <span className={styles.errorSpan}>*{errors.thumbnail}</span>}
              </Grid>
              <Grid item xs={12}>
                  <TextField className={styles.input} fullWidth margin="dense" label="Image to Add" placeholder="https://..." value={input.image} name="image" type="text" onChange={(e) => handleInputChange(e)} />
              </Grid>
              <Grid item xs={12}>
                  <Button disabled={(input.image === '' || errors.image) && "disabled"} onClick={(e) => { handleImageAddition(e) }} color="primary" variant="contained" size="small">Add Image</Button>
                  {errors.image && <span className={styles.errorSpan}>*{errors.image}</span>}
              </Grid>
              <Grid item xs={12}>
                <Typography variant="subtitle1" align="left">Stored Images</Typography>
              </Grid>
              <Grid item xs={12}>
                <Grid container xs={12} direction="row">   
                    {input.images?.map((imagen, index) => {
                      return (
                        <Grid item xs={3} className={styles.imageToAdd} key={index}>
                          <img className={styles.image} src={imagen} alt="to-add" />
                          <Button value={imagen} onClick={(e) => { handleImageDeletion(e) }} color="error" variant="contained" size="small">Delete</Button>
                        </Grid>
                     )
                    })}
                </Grid>
              ...
              <Grid item xs={12}>
                <Button fullWidth onClick={(e) => { handleSubmit(e) }} color="primary" variant="contained" size="small">Modify Product</Button>
              </Grid>
            </Grid>
          </CardContent>
      </Card>
